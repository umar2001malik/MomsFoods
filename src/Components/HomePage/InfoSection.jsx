...
// import video2 from "../../assets/milk_video.webm";
// import image2 from "../../assets/product_images.png";
// import image4 from "../../assets/image4.png";
// import video3 from "../../assets/video_dryFruit.webm";
...
const InfoSection = () => {
  ...
  useEffect(() => {
    ...
    // Preload images and small videos
  ...
  // SIMPLIFIED Animation Variants - Less demanding
  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: { 
        staggerChildren: 0.15, // Increased delay
        delayChildren: 0.3 
      },
    },
  };
  ...
  const textVariants = {
    hidden: { opacity: 0, y: 30 }, // Removed blur and x movement
    visible: {
      opacity: 1,
      y: 0,
      transition: {
  ...
  const videoVariants = {
    hidden: { opacity: 0, scale: 0.95 }, // Removed x movement
    visible: {
      opacity: 1,
      scale: 1,
      transition: {
  ...
  const imageCardVariants = {
    ...
    visible: (i) => ({
      ...
      transition: {
        ...
        stiffness: 80,
        damping: 15,
        delay: 0.8 + i * 0.2, // Increased delay between items
      },
    }),
    hover: {
  ...
  const buttonVariants = {
    ...
    visible: {
      opacity: 1,
      transition: { delay: 1.2 }, // Delayed button
    },
  };
    ...
    <section id="about" className="relative overflow-hidden min-h-screen flex items-center justify-center py-20">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${
            bgVideoReady ? 'opacity-0' : 'opacity-100'
          }`}
        />
      ...
      <div className="relative z-20 container mx-auto px-4 lg:px-8 2xl:px-32 max-w-[2400px]">
        <motion.div
          ...
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }} // Increased margin
          className="flex flex-col lg:flex-row items-center justify-between min-h-[80vh] gap-8 lg:gap-16 2xl:gap-32"
        >
          {/* Left Text Section */}
          <motion.div
          ...
          >
            ...
            {/* Button */}
            <motion.div variants={buttonVariants} className="mt-8 2xl:mt-16 flex justify-center lg:justify-start">
              <motion.button
                onClick={handleLearnMore}
                className="bg-[#8B5E3C] text-white px-6 lg:px-9 py-2.5 lg:py-3.5 rounded-full font-semibold 
                  text-[clamp(0.85rem,1vw,1.1rem)] hover:bg-[#7A4E2C] transition-colors duration-300 shadow-lg"
              >
                {t('infoSection.button.learnMore')}
              </motion.button>
            </motion.div>
          </motion.div>

          {/* Right Media Grid - Only animate when media is loaded */}
          <motion.div
            className="lg:w-1/2 flex justify-center lg:justify-end mt-10 lg:mt-0"
            variants={mediaLoaded ? videoVariants : {}}
            initial="hidden"
            ...
            <div className="grid grid-cols-2 gap-4 lg:gap-6 2xl:gap-8 
              ...
              {mediaItems.map((item, i) => (
                <motion.div
                  key={i}
                  custom={i}
                  variants={mediaLoaded ? imageCardVariants : {}}
                  initial="hidden"
                  animate={mediaLoaded ? "visible" : "hidden"}
                  whileHover="hover"
                  className="bg-white/90 backdrop-blur-md rounded-2xl border border-white/40 shadow-lg overflow-hidden cursor-pointer
                    w-full h-40 sm:h-48 md:h-56 lg:h-64 xl:h-72 2xl:h-80"
                >
                  {item.type === "image" ? (
                    <img
                      src={item.src}
