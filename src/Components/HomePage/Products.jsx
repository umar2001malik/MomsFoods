...
// import backgroundVideo from "../../assets/video5.webm";
// import rightSideVideo from "../../assets/video9.webm";
...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const handleBgVideoCanPlay = () => {
      setBgVideoReady(true);
    };

    const handleRightVideoCanPlay = () => {
      setRightVideoReady(true);
    };

    if (bgVideo) {
      bgVideo.addEventListener('canplay', handleBgVideoCanPlay);
      bgVideo.preload = "auto";
      bgVideo.load();
    }

    if (rightVideo) {
      rightVideo.addEventListener('canplay', handleRightVideoCanPlay);
    ...
    return () => {
      if (bgVideo) {
        bgVideo.removeEventListener('canplay', handleBgVideoCanPlay);
      }
      if (rightVideo) {
        rightVideo.removeEventListener('canplay', handleRightVideoCanPlay);
      }
    };
  }, []);
  ...
  return (
    <section
      ...
      className="relative overflow-hidden min-h-screen flex items-center justify-center py-20 w-full"
    >
      {/* Main Background Video with Fallback */}
      <div className="absolute inset-0 w-full h-full">
        {/* Fallback Image */}
        <img
          src={bgImage}
            ...
            <div className="grid sm:grid-cols-2 gap-6 lg:gap-8 2xl:gap-10 w-full px-0">
              {products.map((product, idx) => (
                <motion.div
                  key={product.id}
                  custom={idx}
                  variants={mediaLoaded ? productCardVariants : {}}
                  initial="hidden"
          ...
          >
            ...
            <div className="absolute inset-0 w-full h-full">
              ...
              <img
                ...
                alt="background"
                className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${
                  rightVideoReady ? 'opacity-0' : 'opacity-100'
                }`}
              />
              ...
              <video
                ...
                preload="auto"
                className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${
                  rightVideoReady ? "opacity-100" : "opacity-0"
                }`}
              >
                <source src={rightSideVideo} type="video/webm" />
                {t('products.videoFallback')}
              </video>
