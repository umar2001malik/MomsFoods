  ...
  const IngredientsModal = () => {
    ...
    return (
      <motion.div
        ...
        animate="animate"
        exit="exit"
        className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-2 sm:px-4 py-4"
        onClick={() => setActiveModal(null)}
      >
        <motion.div
          variants={modalContentVariants}
          className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-md mx-auto my-auto"
          onClick={(e) => e.stopPropagation()}
          style={{ 
        ...
        >
          <div className="flex justify-between items-center p-4 sm:p-6 border-b border-[#e8e4d9] flex-shrink-0">
            <motion.h3 
              className="text-lg sm:text-xl font-serif font-bold text-[#8B5E3C]"
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          <motion.div 
            className="space-y-2 p-4 sm:p-6 overflow-y-auto flex-grow"
            variants={staggerListVariants}
            initial="hidden"
            animate="visible"
            ...
            {displayIngredients.length > 0 ? (
              displayIngredients.map((ingredient, index) => (
                <motion.div
                  key={index}
                  variants={listItemVariants}
                  className="flex items-center space-x-2 sm:space-x-3 text-[#8B5E3C] text-sm sm:text-base py-1"
                  whileHover={{ x: 5, transition: { duration: 0.2 } }}
                >
                  <motion.div 
                    className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-[#A67C52] rounded-full flex-shrink-0"
                    whileHover={{ scale: 1.5 }}
                    transition={{ type: "spring", stiffness: 300 }}
                  />
                  <span>{ingredient}</span>
  ...
  const NutritionModal = () => {
    ...
    return (
      ...
      >
        <motion.div
          variants={modalContentVariants}
          className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-md sm:max-w-2xl mx-auto my-auto"
          onClick={(e) => e.stopPropagation()}
          style={{ 
            maxHeight: 'calc(100vh - 2rem)',
            display: 'flex',
        ...
        >
          ...
          <motion.div 
            className="overflow-x-auto p-4 sm:p-6 flex-grow"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
            style={{ maxHeight: 'calc(80vh - 120px)' }}
